...
const WIDGET_URL = `${PUBLIC_BASE_URL}/widget/`;
...
const mcpServer = new McpServer({ name: "blunder-lens", version: "0.0.1" });

// Register widget as an MCP resource so hosts can discover and fetch it
mcpServer.registerResource(
  ...
  async () => {
    ...
    return {
      contents: [{ uri: WIDGET_URL, mimeType: RESOURCE_MIME_TYPE, text }],
    };
  }
);
  ...
  async (args) => {
    try {
      ...
      return {
        ...
        _meta: { [RESOURCE_URI_META_KEY]: WIDGET_URL },
      };
    } catch (err) {
...
const httpServer = http.createServer(async (req, res) => {
  ...
  // MCP endpoint — stateless transport per request
  if (pathname === "/mcp") {
  ...
  if (pathname.startsWith("/widget")) {
    const relative = pathname.replace(/^\/widget/, "") || "/index.html";
